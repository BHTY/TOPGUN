...
int do_int(i386* pCPU, decoded_op* src) {
	...
	switch (vector) {
		case 0x21:
			switch (fn_code) {
				...
				case 0x09: //display string
					...
					while (bus_read_8(pCPU->ds.base + offset) != '$') {
						printf("%c", bus_read_8(pCPU->ds.base + offset++));
					}
					break;
				default:
					printf("Unsupported MS-DOS API call 0x%02x\n", fn_code);
					dump_386(pCPU);
					while (1);
					break;
			}
		...
		case 0x10:
			switch (fn_code) {
				...
				case 0x09:
					...
					break;
				case 0x00:
					printf("VBIOS: Setting mode %02xh\n", pCPU->al);

					if (pCPU->al != 0x13) while (1);
				...
				case 0x0E:
					...
					break;
				default:
					printf("Unsupported Video BIOS function code 0x%02x\n", fn_code);
					while (1);
					break;
			}
			break;
		case 0x16:
			switch (fn_code) {
				case 0x00:
					...
					break;
				case 0x02:
					//printf("Stubbing Keyboard BIOS function 0x02\n");
					pCPU->al = (ShiftDown()) | (ShiftDown() << 1) | (CtrlDown() << 2) | (AltDown() << 3);
					break;
				default:
					printf("Unsupported keyboard BIOS function code 0x%02x\n", fn_code);
					while (1);
					break;
			}
			break;
		case 0x1A:
			switch(fn_code){
				case 0x00: //stub RTC read
					pCPU->dx = WinReadTimer() / 18.2;
					break;
				default:
					printf("Unsupported RTC function code 0x%02x\n", fn_code);
					dump_386(pCPU);
					while(1);
					break;
			}
		...
		case 0x20:
			...
			break;
		default:
			printf("Unsupported interrupt 0x%02x with function code 0x%02x\n", vector, fn_code);
			while (1);
			break;
	}
